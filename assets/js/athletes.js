...
function render() {
  ...
  matches.forEach((athlete, index) => {
    ...
    link.href = `athlete.html?swimmer_id=${encodeURIComponent(athlete.source_swimmer_id)}`;
    ...
    meta.textContent = [athlete.english_name, athlete.gender, athlete.birth_year ? `${athlete.birth_year} 年生` : ''].filter(Boolean).join(' · ');
...
async function load() {
  if (isSupabaseConfigured()) {
    ...
    const { data, error } = await supabase.from('athletes')
      .select('source_swimmer_id,full_name,english_name,gender,birth_year').eq('active', true).order('full_name');
    if (error) throw error;
    athletes = (data || []).filter(athlete => athlete.source_swimmer_id);
  } else {
    ...
    athletes = [...unique.values()].sort((a, b) => String(a.full_name).localeCompare(String(b.full_name), 'zh-Hant'));
  }
  render();
}
